...
func _do_charge_fire(spawn_pos: Vector2, charge_level: float) -> void:
	var count: int = 5 if charge_level < 0.6 else (8 if charge_level < 1.0 else 11)
	var spread_angle: float = 10.0 if charge_level < 0.6 else (16.0 if charge_level < 1.0 else 22.0)
