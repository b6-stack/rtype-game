...
func _phase_attack(delta: float) -> void:
	...
	match current_phase:
		...
		1:
			if _pattern_timer >= 0.95:
				_pattern_timer = 0.0
				_fire_radial(10, 480.0, 2, Color.ORANGE, _time * 45.0)
				_fire_at_player(680.0, 3, Color.RED)
		2:
			if _pattern_timer >= 0.75:
				_pattern_timer = 0.0
				_fire_radial(12, 500.0, 2, Color.CYAN, -_time * 60.0)
				_fire_radial(6, 370.0, 2, Color.MAGENTA, _time * 30.0)
		3:
			if _pattern_timer >= 0.55:
				_pattern_timer = 0.0
				_fire_radial(14, 540.0, 3, Color.YELLOW, sin(_time * 3.0) * 90.0)
